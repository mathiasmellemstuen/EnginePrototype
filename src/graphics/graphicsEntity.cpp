...
GraphicsEntity& createGraphicsEntity(Renderer& renderer, RenderPassObject* renderPassObject, Shader* shader, VertexBuffer* vertexBuffer, Texture* texture, bool enableDepthTest, unsigned int pushConstantsSize) {
    ...
    if (vkCreateDescriptorSetLayout(renderer.device, &layoutInfo, nullptr, &graphicsEntity->descriptorSetLayout) != VK_SUCCESS) {
    ...
    if (vkCreatePipelineLayout(renderer.device, &pipelineLayoutInfo, nullptr, &graphicsEntity->pipelineLayout) != VK_SUCCESS) { 
...
void freeGraphicsEntity(Renderer& renderer, GraphicsEntity& graphicsEntity) {
    vkDestroyPipeline(renderer.device, graphicsEntity.graphicsPipeline, nullptr);
    vkDestroyPipelineLayout(renderer.device, graphicsEntity.pipelineLayout, nullptr);
    vkDestroyDescriptorSetLayout(renderer.device, graphicsEntity.descriptorSetLayout, nullptr);
