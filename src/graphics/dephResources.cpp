...
void DepthResources::create(PhysicalDevice& physicalDevice, LogicalDevice& logicalDevice, ImageViews& imageViews, SwapChain& swapChain) {
    ...
    createImage(physicalDevice, logicalDevice, swapChain.swapChainExtent.width, swapChain.swapChainExtent.height, depthFormat, VK_IMAGE_TILING_OPTIMAL, VK_IMAGE_USAGE_DEPTH_STENCIL_ATTACHMENT_BIT, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, depthImage, depthImageMemory);

    depthImageView = imageViews.createImageView(logicalDevice, depthImage, depthFormat, VK_IMAGE_ASPECT_DEPTH_BIT);
